...
export const MESSAGES: { zh: MessageDict; en: MessageDict } = {
  zh: {
    ...
    'sys.speakTimedOut': () => '(发言超时已停止,可重发消息继续)',
    'sys.noOutput': () => '(无输出)',
    ...
    'room.modelUpdated': (p) => `已切换 ${p?.name ?? ''} 的模型为 ${p?.model ?? ''}(下次发言生效)`,
    'room.compactDone': (p) => `已压缩 ${p?.name ?? ''} 的会话上下文(/compact)`,
    'room.compactFailed': (p) => `${p?.name ?? ''} 的会话压缩失败(可稍后重试)`,
    'room.compactNoSession': (p) => `${p?.name ?? ''} 尚无可压缩的会话(先发言产生会话后再 /compact)`,
    ...
    'api.extraArgsNotAllowed': (p) => `CLI 附加参数含非白名单项(仅允许 --model 与 --thinking-budget): ${p?.args ?? ''}`,
    'api.taskNeedsProject': () => '任务房必须绑定一个本机存在的项目目录',
    'api.taskDegraded': (p) => `导入的房间「${p?.name ?? ''}」原为任务房,但其项目路径本机不存在(${p?.path ?? ''}),已降级为聊天房导入。可在设置中重新绑定项目。`,
  ...
  en: {
    ...
    'sys.speakTimedOut': () => '(speech timed out — resend a message to continue)',
    ...
    'room.modelUpdated': (p) => `Switched ${p?.name ?? ''} to model ${p?.model ?? ''} (effective on next speech)`,
    'room.compactDone': (p) => `Compacted ${p?.name ?? ''}'s session context (/compact)`,
    'room.compactFailed': (p) => `Failed to compact ${p?.name ?? ''}'s session (try again later)`,
    'room.compactNoSession': (p) => `${p?.name ?? ''} has no session to compact yet (speak once first, then /compact)`,
    ...
    'api.extraArgsNotAllowed': (p) => `CLI extra args contain non-allowlisted items (only --model and --thinking-budget are allowed): ${p?.args ?? ''}`,
    'api.taskNeedsProject': () => 'A task room must bind to a project directory that exists on this machine',
    'api.taskDegraded': (p) => `Imported room "${p?.name ?? ''}" was a task room, but its project path does not exist locally (${p?.path ?? ''}); it was imported as a chat room instead.`,
