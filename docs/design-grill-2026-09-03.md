...
### 3. 管理员(Scout 改名扩职) [已完成 ✅ (2026-09-05)]

- `scout.ts` → `admin.ts`,「🔍 侦察员」→「管理员」
- **架构不变**:内置角色、不占成员席、不被编排选中、agents.yaml 的 scout 段改名 admin 段(便宜模型+超时+熔断闩)
- **职责**:① 项目预分析(原侦察,原样) ② 讨论摘要(新)
- **落地记录**:已落地 `admin.ts`，支持 `ensureScout` 与 `generateSummary` 并带超时和熔断闩防御。

### 4. 讨论摘要(管理员职责二) [已完成 ✅ (2026-09-07 演进为上下文压缩层)]

> **演进说明与落地记录**:
> - 2026-09-07 重构方案升维为**上下文压缩层**（链式滚动合并公聊摘要 + stateless 私聊第一人称自纪要 + stateful session 原生 /compact + 锚点滑动视窗组装）；
> - 摘要与纪要独立快照落盘并经 `summaryChain` 槽位化合并，解决多写者互吞；
> - 前端右侧 Inspector 独立呈现摘要文件列表、公聊大纲与成员私聊纪要。

- **触发**:自动 + 手动双入口
  - 自动:公聊未覆盖超过阈值(默认 30 条)落库时异步触发；私聊未覆盖超阈值触发成员自纪要
  - 手动:Inspector 面板「生成摘要」按钮即时提炼最新快照
- **存储**:独立文件快照存储(`data/summaries/`)，重启还在，buildPrompt 锚点视窗精准注入
- **语义**:prompt 层替代——最新摘要+锚点后增量，旧消息不删(UI/导出仍见完整历史)
- **失败**:优雅降级照旧注入，熔断闩复用，绝不阻塞发言链路
...
### 7. 成本仪表盘(轻量版) [已完成 ✅ (2026-09-05)]

> **落地记录**:
> - 前后端完整支持：后端由 `store/trace.ts` 提供 `computeSessionStats`，计算 Token 用量、USD 开销与平均耗时；
> - 前端由侧栏 Inspector 的 `InspectorStats.vue` 呈现 4 格核心 KPI 仪表盘与角色明细用量排行榜（按发言与 Token 排序，含活跃份额与跳过/错误统计）。
...
| B3 | 成本仪表盘 + markdown 导出(前端 Blob) | 零依赖,随时可插 | 成本仪表盘已完成 ✅ (2026-09-05); markdown 导出待办 |
